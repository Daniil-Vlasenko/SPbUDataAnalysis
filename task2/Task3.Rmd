...
Попробуем сделать предсказание для шерстистого мамонта (вымер).
Сложно сказать какой индекс PRED_IND должен быть у капибар, это самый крупный 
грызун размером с овцу, не хишник, но и врагов в природе не много. Пусть будет 3.
```{r}
new <- data_frame(BRAIN_WE.log=c(log(55)), PRED_IND=c(3)) %>% mutate(PRED_IND = as.factor(PRED_IND))
exp(predict(model5.dum, new))
```

На деле капибары живут 8-10 лет на воле и 10-12 в неволе.
