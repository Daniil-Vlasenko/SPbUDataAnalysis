...
C одной стороны, доверительная область хороша тем, что по ней видна слабая 
корреляция коэффициентов модели, с другой - она плохая, так как PRED_IND может 
быть незначим.
```{r}
confint(model5.num.beta)
```

Посмотрим на оставшиеся два признака.
```{r}
...
Между признаками есть нелинейная зависимость, поэтому коэффициент корреляции 
Пирсона близок к нулю. Зависимость выглядит как парабола. Давайте посмотрим 
какая будет значимость у модели, если PRED_IND сделать dummy variable.
```{r}
...
```
 ...
 Модель хуже той, что мы строили с BRAIN_WE.log и DANG_IND с dummy variable по 
 p-value и multiple R-squared, так что дальше будем работать с нашей моделью а 
 не моделью AIC.
...
tmp <- data_frame(LIFESPAN=dfNew.o.dum$LIFESPAN, residuals=residuals(model.AIC))
ggplot(tmp, aes(x=LIFESPAN, y=residuals)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE)

mean <- mean(tmp$residuals)
std <- sqrt(var(tmp$residuals))
...
ggplot(tmp %>% filter(LIFESPAN<55), aes(x=LIFESPAN, y=residuals)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE)
...
За 3 сигмы нет выбросов, есть только 4 выбросов за 2 сигмы. Посмотрим, что
это за индивиды.
```{r}
dfNew.o.num[tmp$residuals > mean+2*std | tmp$residuals < mean-2*std,]$NAME
dfNew.o.num[tmp$residuals > mean+2*std | tmp$residuals < mean-2*std,]
```

Большая летучая коричневая мышь, ехидна, маленькая летучая коричневая мышь, тупайи.

11\. Посмотрим на Residuals vs Deleted Residuals plot.
Красная линия - это линия y=x, а синия линий - это линия регрессии.
```{r, message=FALSE, warning=FALSE}
df.rs <- data_frame(rstandard=rstandard(model.AIC), rstudent=rstudent(model.AIC))
