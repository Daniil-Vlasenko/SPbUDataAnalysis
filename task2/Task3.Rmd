...
вычислим стандартизованные коэффициенты. 
```{r}
library(lm.beta)
...
На эллипсоиде можно заметить, что корреляция между коэффициентами слабая, что 
...
PRED_IND как dummy variable, чтобы разные его значения могли вносить вклад своими 
...
Давайте посмотрим, какая будет значимость у модели, если PRED_IND сделать dummy variable.
```{r}
...
Проведем backward AIC для данных без и с dummy variable.
```{r, message=FALSE}
library(MASS)
...
Проведем forward AIC для данных без и с dummy variable.
```{r}
model2.num.AIC <- lm(LIFESPAN.log~1, data=na.omit(dfNew.o.num))
stepAIC(model2.num.AIC, direction="forward", scope=list(upper=model1.AIC,lower=model2.AIC))
```
